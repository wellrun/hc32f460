...
{
    if(true == EMB_GetStatus(M4_EMB1, EMBFlagPortIn))
    {
        M4_PORT->PODRE_f.POUT06 = 1;

        EMB_SwBrake(M4_EMB1, true);  //Software brake Enable, still shunt down PWM after Clear Port In Brake

        if(0 == EMB_GetStatus(M4_EMB1, EMBPortInState))
        {
            EMB_ClrStatus(M4_EMB1, EMBPortInFlagClr);  //Clear Port In Brake
...
int32_t main(void)
{
    ...
    stcEMBConfigCR.bEnPortBrake = true;
    stcEMBConfigCR.bEnPorInFlt  = true;
    stcEMBConfigCR.enPortInFltClkSel = EMBPortFltDiv32;
    stcEMBConfigCR.bEnPortInLevelSel_Low = true;
    EMB_Config_CR_Timer6(&stcEMBConfigCR);

    EMB_ClrStatus(M4_EMB1, EMBPortInFlagClr);  //Clear Port In Brake

    EMB_ConfigIrq(M4_EMB1, PORTBrkIrq, true);
