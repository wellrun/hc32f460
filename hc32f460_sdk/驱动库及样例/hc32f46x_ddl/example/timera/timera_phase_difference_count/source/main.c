...
/** \file main.c
 **
 ** \brief The example of Timera phase difference count function
 **
...
#define TIMERA_UNIT1_UPDATE_INT         INT_TMRA1_UDF
 ...
 **
 ******************************************************************************/
void TimeraUnit1Under_IrqCallback(void)
{
    LED1_TOGGLE();
...
void Timera_Config(void)
{
    ...
    TIMERA_IrqCmd(TIMERA_UNIT1, TimeraIrqUnderflow, Enable);

    /* Configure coding count structure */
    stcTimeraCondingInit.enIncClkBHighAndClkARisingEn = Enable;
    stcTimeraCondingInit.enDecClkBLowAndClkARisingEn = Enable;
    ...
    stcIrqRegiConf.enIntSrc = TIMERA_UNIT1_UPDATE_INT;
    stcIrqRegiConf.enIRQn = Int007_IRQn;
    stcIrqRegiConf.pfnCallback = TimeraUnit1Under_IrqCallback;
    enIrqRegistration(&stcIrqRegiConf);
    NVIC_ClearPendingIRQ(stcIrqRegiConf.enIRQn);
    NVIC_SetPriority(stcIrqRegiConf.enIRQn, DDL_IRQ_PRIORITY_15);
    NVIC_EnableIRQ(stcIrqRegiConf.enIRQn);
...
/**
 *******************************************************************************
 ** \brief  main function for Timera phase difference count function
 **
 ** \param [in]  None
 **
