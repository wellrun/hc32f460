 ...
 ******************************************************************************/
static uint16_t m_au16Data0Val[4];
static uint16_t m_au16Data2Val[4];
static uint16_t m_au16Data1Val[4] = {0x0000, 0x2222, 0x4444, 0x8888};
...
int32_t main(void)
{
    ...
    uint8_t u8CalTimes = sizeof(m_au16Data1Val)/2;
    ...
    {
        DCU_WriteDataHalfWord(DCU_UNIT, DcuRegisterData1, m_au16Data1Val[i]);

        m_au16Data0Val[i] = DCU_ReadDataHalfWord(DCU_UNIT, DcuRegisterData0);
        m_au16Data2Val[i] = DCU_ReadDataHalfWord(DCU_UNIT, DcuRegisterData2);

        /* Compare DCU regisger DATA0 && DATA2 value: DATA0 value == 2 * DATA2 value */
        if (m_au16Data0Val[i] != (2 * m_au16Data2Val[i]))
        {
            enTestResult = Error;
            break;
        }
