 ...
 ******************************************************************************/
/*!< Parameter valid check for standard clock */
#define IS_VALID_STANDARD_CLK(x)                ((x) <= 0x3F)
...
en_flag_status_t I2S_GetStatus(M4_I2S_TypeDef* pstcI2sReg, en_i2s_std_t enStd)
{
    ...
    return (en_flag_status_t)(pstcI2sReg->SR & (1 << (uint8_t)enStd));
}
...
void I2S_ClrErrFlag(M4_I2S_TypeDef* pstcI2sReg, en_i2s_err_flag_t enErrFlag)
{
    ...
    pstcI2sReg->ER &= ~(1 << (uint8_t)enErrFlag);
}
                                ...
                                en_i2s_err_flag_t enErrFlag)
{
    ...
    return (en_flag_status_t)(pstcI2sReg->ER | (1 << (uint8_t)enErrFlag));
}
...
void I2s_Init(M4_I2S_TypeDef* pstcI2sReg, const stc_i2s_config_t* pstcI2sCfg)
{
    ...
    *(uint32_t*)&stcCTRL_Tmp = pstcI2sReg->CTRL;
    ...
    if(Enable == pstcI2sCfg->enExckEn)
    {
        ...
        stcCTRL_Tmp.CLKSEL = 1;
        stcCTRL_Tmp.I2SPLLSEL = 0;
    ...
    else
    {
        ...
        stcCTRL_Tmp.CLKSEL = 0;
        stcCTRL_Tmp.I2SPLLSEL = 1;
        /* Get i2c clock frequency */
        CLK_GetClockFreq(&stcClkFreq);
        i2sclk = stcClkFreq.sysclkFreq;
    }
    ...
    if(true == pstcI2sCfg->enMcoOutEn)
    {
        ...
        tmp = i2sclk/(pstcI2sCfg->u32AudioFreq * u8ChanelDataBit * 2 * u8ChanMul);
    }
    else
    {
        /* MCLK output is disabled */
        tmp = i2sclk/(pstcI2sCfg->u32AudioFreq * u8ChanelDataBit * 2);
    }
    i2sodd = tmp & 0x0001;
    i2sdiv = (tmp - i2sodd) / 2;
