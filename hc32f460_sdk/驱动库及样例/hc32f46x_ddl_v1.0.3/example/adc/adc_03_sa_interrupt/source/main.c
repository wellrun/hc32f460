...
int32_t main(void)
{
    ...
    /* Check ADC1 interrupt. */
    while (!(m_u32AdcIrqFlag & ADC1_SA_IRQ_BIT));

    /* Start ADC2. */
    ADC_StartConvert(M4_ADC2);
    /* Check ADC2 interrupt. */
    while (!(m_u32AdcIrqFlag & ADC2_SA_IRQ_BIT));
    ...
    /***************************** Usage 2 *******************************/
        ...
        if (m_u32AdcIrqFlag & ADC1_SA_IRQ_BIT)
        {
            ...
            printf("\nADC1 sequence A interrupt occurred.");
            // TODO: YOUR CODE.
        }

        /* ADC2 sequence A will be triggered by timer0 every second. */
        if (m_u32AdcIrqFlag & ADC2_SA_IRQ_BIT)
        {
            m_u32AdcIrqFlag &= ~ADC2_SA_IRQ_BIT;
            printf("\nADC2 sequence A interrupt occurred.");
            // TODO: YOUR CODE.
        }
    }
}
...
static void AdcChannelConfig(void)
{
    ...
    ADC_SeqITCmd(M4_ADC2, AdcSequence_A, Enable);
}
...
#if (ADC1_TRIGGER_MODE == EXTERNAL_PIN_TRIGGER)
    /*
     *  ADC1:
...
#if (ADC2_TRIGGER_MODE == INTERNAL_EVENT_TRIGGER)
    /*
